...
void* ventanilla(void* estructura_hilo){
	...
	while(estado == ANTES){
		if(msgrcv(((Estructura_hilo*)estructura_hilo)->id_mensajes , (struct msgbuf*)&apuesta, sizeof(apuesta)-sizeof(long), 1, IPC_NOWAIT)>0){
			...
			sprintf(tmp, "%s %d %d %f %lf\n", apuesta.apostador, (int)apuesta.id, apuesta.caballo, compartida->cotizacion[apuesta.caballo],apuesta.apuesta);
			printf("%s\n", tmp);
...
int main(int argc, char const *argv[]){
	...
	if(pid_apostador == 0){
		...
		if(signal(SIGUSR1, manejador_final) == SIG_ERR){
			printf("Error en el manejador final en el apostador\n");
			exit(EXIT_FAILURE);
		}

		for(i = 0; i < n_apostadores; i++){
			sleep(1);
			apostador(i+1, cantidad, n_caballos, id_mensajes);
		}
		exit(EXIT_SUCCESS);
	/*Proceso principal*/
